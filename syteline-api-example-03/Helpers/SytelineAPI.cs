    ...
    public APILoadCollectionResponse LoadCollection(string idoName, List<string> properties, string? filter = null, List<OrderByProperty>? orderBy = null, int? recordCap = null, bool? distinct = null, string? clm = null, List<string>? clmParam = null, string? pqc = null, bool? readOnly = null, string paginationMode = "bookmark", string paginationProperty = "RowPointer")
    {
        ...
        do
        {
            ...
            if (clm != null && clm != "")
            {
                ...
                // MAKE SURE CLM PARAMS ARE SET

                if (clmParam == null || clmParam.Count == 0)
                {
                    batchCLMParam = ["", "", requestCap.ToString()];
                }
                else if (clmParam.Count == 1)
                {
                    batchCLMParam = [clmParam[0], "", requestCap.ToString()];
                }
                else
                {
                    batchCLMParam = [clmParam[0], clmParam[1], requestCap.ToString()];
                }
            ...
            if (haveToPaginate && paginationMode == "propertyBased")
            {
                ...
                if (clm != null && clm != "")
                {
                    batchCLMParam[1] = paginationProperty + " ASC";
                }
                else
                {
                ...
                if (!isFirstQuery)
                {
                    ...
                    if (clm != null && clm != "")
                    {
                        ...
                        string paginationFilter = $"( {paginationProperty} > '{(paginationProperty == "RowPointer" ? "{" + paginationPropertyHighestValue + "}" : paginationPropertyHighestValue)}' )";

                        batchCLMParam[0] = (batchCLMParam[0] != "" ? $"( {batchCLMParam[0]} ) AND " : "") + paginationFilter;
